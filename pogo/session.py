...
class PogoSession(object):
    ...
    def wrap_in_request(self, payload, defaults=True):
        ...
        # Build Envelope
        latitude, longitude, altitude = self.get_coordinates()
        ...
        # Add requests
        if defaults:
            payload += self.get_defaults()
        req.requests.extend(payload)

        return req

    def request_or_throw(self, req, url=None):
        if url is None:
            url = self.endpoint
    ...
    def request(self, req, url=None):
        try:
            return self.request_or_throw(req, url)
        except Exception as e:
            logging.error(e)
            raise GeneralPogoException('Probably server fires.')

    def wrap_and_request(self, payload, defaults=True):
        res = self.request(self.wrap_in_request(payload, defaults=defaults))
        if defaults:
    ...
    def check_download_settings(self):
        return self.state.settings
    ...
    def get_stats(self):
        payload = [Request_pb2.Request(
            request_type=RequestType_pb2.GET_PLAYERSTATS
        )]
        res = self.wrap_and_request(payload)
        self.state.stats.ParseFromString(res.returns[0])

        return self.state.stats

    # Get Location
    def get_map_objects(self, radius=10):
    ...
    def catch_pokemon(self, pokemon, pokeball=1):
    ...
    # Throw away items
    def recycle_item(self, item_id, count):
    ...
    def encounter_and_catch(self, pokemon, pokeball=1, delay=2):
        self.encounter_pokemon(pokemon)
        time.sleep(delay)
        return self.catch_pokemon(pokemon, pokeball)
